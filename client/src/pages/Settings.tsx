...
const PP = { bg:'#EEF2F7', white:'#FFFFFF', navy:'#003087', blue:'#0070BA', text:'#1D1D1D', sub:'#4A4F54', muted:'#6C7378', border:'#CBD2D9', green:'#00A651' };

export default function Settings() {
  const { user, logout } = useAuth();
  const { openChat } = useChatContext();
  const items = [
    { icon: User,  label:'الملف الشخصي',       desc:'تحديث معلومات الملف الشخصي',         ibg:'#EBF4FB', ic:PP.blue  },
    { icon: Bell,  label:'الإشعارات',           desc:'إدارة تنبيهات التطبيق',               ibg:'#EDF3FF', ic:PP.navy  },
    { icon: Lock,  label:'الخصوصية والأمان',    desc:'تحديث كلمة المرور والأمان',           ibg:'#EBF4FB', ic:PP.blue  },
    ...
    <AppLayout onChatOpen={openChat}>
      <div className="pb-28" style={{background:PP.bg,minHeight:'100vh'}}>
        ...
        <motion.div initial={{opacity:0,y:-16}} animate={{opacity:1,y:0}}
          ...
          style={{background:PP.navy,boxShadow:'0 4px 18px rgba(0,48,135,0.25)'}}>
          ...
          {items.map((item,i) => {
            ...
            return (
              <motion.button key={item.label} initial={{opacity:0,x:-16}} animate={{opacity:1,x:0}}
                ...
                className="w-full flex items-center gap-4 px-5 py-4 text-right transition-colors active:bg-[#EEF2F7]"
